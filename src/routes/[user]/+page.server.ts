...
import type { SpotifyPlaylist, SpotifyPlaylistTracksResponse, SpotifyUserPlaylistsResponse } from '$lib/types/api.js';
...
const fetchClientToken = async () => {
  ...
  const fetchUserPlaylists = async (accessToken: string, spotifyUserId: string): Promise<SpotifyUserPlaylistsResponse> => {
  ...
  const findRecommendPlaylist = (playlists: SpotifyPlaylist[]): SpotifyPlaylist  => {
	const match = playlists.find((playlist: SpotifyPlaylist) => playlist.name === recommendedPlaylistName);
  ...
  const fetchPlaylistTracks = async (accessToken: string, playlistId: string): Promise<SpotifyPlaylistTracksResponse> => {
